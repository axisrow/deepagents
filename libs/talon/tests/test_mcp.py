...
def test_discover_mcp_config_paths_requires_non_empty_mcp_servers(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    config = TalonConfig.from_env({"AGENT_ASSISTANT_ID": "test"}, base_home=tmp_path)
    config.ensure_home()
    invalid_path = config.manifest_dir / ".mcp.json"
    invalid_path.write_text('{"mcpServers": {}}', encoding="utf-8")

    assert discover_mcp_config_paths(config) == []


def test_discover_mcp_config_paths_skips_malformed_json(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
